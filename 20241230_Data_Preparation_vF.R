...
write.csv2(ILS, file = "ILS.TXT", row.names = FALSE)
write.csv2(FirmsAll, file = "FirmsAll.TXT", row.names = FALSE)
...
FirmsAll <- read_csv2("FirmsAll.TXT")
...
# Filter to June 2018
ILS_f <- ILS %>% filter(Year.y <= 2018)

# count unique individuals and firms
length(unique(ILS_f$ID_Company))
length(unique(ILS_f$WorkerID))
...
ILS <- ILS %>% mutate(  WorkTime = as.factor(WorkTime),
                        ...
                        FirmPosition = as.factor(FirmPosition.x),
                        SizeClass = as.numeric(SizeClass.x)) 
...
ILS <- ILS %>% mutate(ATECO07 = ATECO07.x)
...
write.csv2(ILSH, file = "ILShirings.TXT", row.names = FALSE)
...
ILSH <- ILSH %>% mutate(AgeGroup = 
                          ifelse(Age < 35, "Young",  # 15-34 years old
                                 ifelse(Age < 55, "Prime Age", # 35-54 years old 
                                        "Senior Worker" )))
...
ILSH <- ILSH %>% mutate(WorkTime =as.factor(WorkTime),
                        ...
                        MotherFirmID = as.factor(MotherFirmID.x),
